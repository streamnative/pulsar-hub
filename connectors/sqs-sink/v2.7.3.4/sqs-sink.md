...
tags: 
alias: alias: AWS SQS Sink Connector
features: [""]
icon: /images/connectors/sqs-logo.png
download: "https://api.github.com/repos/streamnative/pulsar-io-sqs/tarball/refs/tags/v2.7.3.4"
...
![](https://raw.githubusercontent.com/streamnative/pulsar-io-sqs/v2.7.3.4/docs/sqs-sink.png)

# How to get 

You can get the SQS sink connector using one of the following methods:

- Download the NAR package from [here](https://github.com/streamnative/pulsar-io-sqs/releases/download/v2.7.3.4/pulsar-io-sqs-2.7.3.4.nar).
   ...
   ```json
    {
        ...
        "archive": "connectors/pulsar-io-sqs-2.7.3.4.nar",
        "parallelism": 1,
        "configs":
        {
...
--archive pulsar-io-sqs-2.7.3.4.nar \
...
2. Copy the NAR package of the SQS connector to the Pulsar connectors directory.
    ...
    ```
    cp pulsar-io-sqs-2.7.3.4.nar PULSAR_HOME/connectors/pulsar-io-sqs-2.7.3.4.nar
    ```
...
1. Build a new image based on the Pulsar image with the SQS sink connector and push the new image to your image registry. This example tags the new image as `streamnative/pulsar-sqs:2.7.3.4`.

    ```Dockerfile
    FROM apachepulsar/pulsar-all:2.7.3.4
    RUN curl https://github.com/streamnative/pulsar-io-sqs/releases/download/v2.7.3.4/pulsar-io-sqs-2.7.3.4.nar -o /pulsar/connectors/pulsar-io-sqs-2.7.3.4.nar
    ```
...
3. Replace the `images` section in the `pulsar.yaml` file with the `images` section of `streamnative/pulsar-sqs:2.7.3.4`.
