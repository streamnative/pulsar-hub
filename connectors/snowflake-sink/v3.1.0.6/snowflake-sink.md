...
- Download the NAR package
  from [the download page](https://github.com/streamnative/pulsar-io-snowflake/releases/download/v3.1.0.6/pulsar-io-snowflake-3.1.0.6.nar)
  .
...
2. Build the connector in the `pulsar-io-snowflake` directory.
   ...
   ```bash
   ls target
   pulsar-io-snowflake-3.1.0.6.nar
   ```
   ...
   ```json
    {
        ...
        "namespace": "default",
        "name": "snowflake-sink",
        "archive": "connectors/pulsar-io-snowflake-3.1.0.6.nar",
        "inputs": [
...
spec:
  image: streamnative/pulsar-io-snowflake:3.1.0.6
  replicas: 1
  maxReplicas: 1
  retainOrdering: true
  input:
  ...
  resources:
    ...
    requests:
      ...
      memory: 1G
  java:
    jar: connectors/pulsar-io-snowflake-3.1.0.6.nar
...
1. Copy the NAR package to the Pulsar connectors directory.
    ...
    ```
    cp pulsar-io-snowflake-3.1.0.6.nar PULSAR_HOME/connectors/pulsar-io-snowflake-3.1.0.6.nar
    ```
